...
HQL_INSERT_ALL="insert overwrite table $t_cp_all partition(date='$day_01') select * from (select a.uuid as platformUuid,a.deviceTypeSid,a.deviceTypeCode,a.deviceTypeName,a.platContentTypeSid,a.platContentTypeCode,a.platContentTypeName,a.platContentDetailApiSid,a.detailApiCode,a.detailApiName,a.isSuccess as platformIsSuccess,b.uuid as sessionUuid,b.dateTime as sessionDateTime,b.sid,b.uid,b.aid,b.lid,b.lty,b.did,
b.dty from (select * from $t_cp_platform where date='$day_01') a left outer join (select * from $t_cp_session where date='$day_01') b on a.uuid=b.uuid) ps
left outer join (select uuid as providerUuid,dateTime as providerDateTime,providerSid,providerCode,providerName,providerContentApiAdapterSid,providerContentApiAdapterCode,providerContentApiAdapterName,cpApiPath,isSuccess as providerIsSuccess from (select *,row_number() over(partition by uuid order by datetime) as num from $t_cp_provider where date='$day_01') t where t.num = 1) p on ps.platformUuid = p.providerUuid"
echo $HQL_INSERT_ALL
/usr/bin/hive -e "$HQL_INSERT_ALL"
